...
const ARCServices = () => {
    ...
    const fetchData = async () => {
        ...
        const servicesCollection = collection(db, 'archived_services');
        ...
        for (const serviceDoc of servicesSnapshot.docs) {
            ...
            // Se for por `carId`, filtramos apenas esse carro
            if (carId !== "0" && serviceData.carId !== carId) continue;

            // Se for por `clientId`, filtramos pelos carros do cliente
    ...
    const toggleStatus = async (id, currentStatus) => {
        ...
        try {
            await updateDoc(doc(db, 'archived_services', id), { status: newStatus });
            ...
            setServices(prevServices => {
                ...
                // Se o serviço NÃO estiver completo OU NÃO estiver pago, desarquiva
                if (updatedService.status !== 'completed' || updatedService.paid !== true) {
                    unarchiveService(updatedService);
                }

                return updated;
            });
        } catch (error) {
            console.error("Erro ao atualizar status (arquivado):", error);
        }
    };
    ...
    try {
        // Atualiza em archived_services
        await updateDoc(doc(db, 'archived_services', id), { paid: newPaid });

        // Atualiza também em allservices
        ...
        console.error("Erro ao atualizar pagamento (arquivado):", error);
    }
    };

    const unarchiveService = async (service) => {
        const { id, carName, clientName, ...data } = service;

        try {
            await setDoc(doc(db, 'services', id), data);
            await deleteDoc(doc(db, 'archived_services', id));
            setServices(prev => prev.filter(s => s.id !== id));
            console.log(`Serviço ${id} desarquivado.`);
        } catch (error) {
    ...
    if (window.confirm(`Deseja excluir o serviço "${service}"?`)) {
        try {
        // Apaga da coleção 'archived_services'
        await deleteDoc(doc(db, 'archived_services', id));

        // Apaga também da coleção 'allservices'
    ...
    return (
        <div className="h-screen">
            ...
            <div className="container mx-auto p-4">
                <h1 className="text-2xl font-bold mb-4">Serviços Arquivados</h1>
                {carId && car && carId !== "0" && (
                    <div className="mb-4">
                    ...
                    {loading ? (
                        <tr>
                            <td colSpan="8" className="border px-4 py-2 text-center">
                                A carregar serviços arquivados{loadingDots} <br />
